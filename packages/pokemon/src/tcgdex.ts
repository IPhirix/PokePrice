...
let _setCache: Map<
  string,
  { name: string; series: string; releaseDate: string; logo: string | null }
> | null = null;
let _setCacheTime = 0;
...
function appendWebp(url: string | undefined | null): string | null {
  return url ? `${url}.webp` : null
}

export async function getSetMap(): Promise<
  Map<
    string,
    { name: string; series: string; releaseDate: string; logo: string | null }
  >
  ...
  _setCache = new Map(
    (res.data || []).map(
      (s: {
        ...
        {
          name: s.name,
          series: s.serie?.name || seriesFromSetId(s.id),
          releaseDate: s.releaseDate || "",
          logo: appendWebp(s.logo),
        },
      ],
    ),
  );
...
export async function searchCards(query: string): Promise<TcgCard[]> {
  ...
  return (res.data || [])
    ...
    .filter(
      (c: TcgCard & { set?: { series?: string; id?: string } }) =>
        !isPocketCard(c),
    );
}
...
export async function getCard(cardId: string): Promise<TcgCard | null> {
  try {
    const res = await axios.get(`${TCGDEX_BASE}/cards/${cardId}`, {
      timeout: 15_000,
    });
...
export async function searchCardsAdvanced(
  ...
  return (
    ...
    .map((card) => {
      ...
      return mapTcgdexCard({
        ...card,
        set: { id: sId, name: setData.name, series: setData.series },
      });
    })
    .filter(
      (c: TcgCard & { set?: { series?: string; id?: string } }) =>
