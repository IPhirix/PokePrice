...
function WidgetTooltip({
  ...
  const displayLabel =
    labelKey === "date" && rawLabel ? fmtDate(rawLabel) : rawLabel;
      ...
      {addedCards?.length > 0 && (
        <div className="mt-1 pt-1 border-t border-surface-600">
          {addedCards.slice(0, 3).map((name, i) => (
            <p
              key={i}
              className="text-slate-300 text-[10px] leading-tight truncate"
            >
              {name}
            </p>
          ))}
...
function WidgetSparkline({
  ...
  if (zeroLine) {
    ...
    const zeroPct =
      range > 0 ? ((domainMax / range) * 100).toFixed(1) + "%" : "0%";
      ...
      >
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p
              className={`text-3xl font-bold ${tile.valueColor || "text-white"}`}
            >
              {tile.currentValue}
            </p>
            {tile.subtitle && (
              <p className="text-slate-500 text-xs mt-1">{tile.subtitle}</p>
            )}
          </div>
          <button
            onClick={onClose}
...
function Widget({
  ...
  return (
    <div
      className={`relative bg-surface-800 border border-surface-600 rounded-xl p-2 flex gap-3 overflow-hidden ${onExpand ? "cursor-pointer hover:border-surface-500 transition-colors" : ""}`}
      onClick={onExpand}
    >
      <div className="w-36 flex-shrink-0 flex flex-col justify-start overflow-hidden pl-3">
        <p className="text-slate-500 text-xs mb-1 uppercase tracking-wider font-medium leading-tight">
          {label}
        </p>
...
export default function PortfolioSummary({
  ...
  const fmt = (n) =>
    hideValues ? "——" : n != null ? format(Math.abs(n)) : "—";

  useEffect(() => {
    window.api.getPortfolio(binderFilter || null).then(setData);
  }, [refreshKey, binderFilter]);

  const dayPos = data.totalDayChange > 0,
    dayNeg = data.totalDayChange < 0;
  const dayColor = dayPos
    ? "text-emerald-400"
    : dayNeg
      ? "text-red-400"
      : "text-slate-400";
  ...
  const combinedPnL =
    data.totalProfit != null || data.realizedPnL != null
      ? (data.totalProfit ?? 0) + (data.realizedPnL ?? 0)
      : null;
  ...
  const plHistory =
    data.totalInvested != null && vh.length > 1
      ? vh.map((p) => ({
          date: p.date,
          value:
            Math.round(
              (p.value - data.totalInvested + (data.realizedPnL ?? 0)) * 100,
            ) / 100,
        }))
      : [];
  const roiHistory =
    data.totalInvested != null && vh.length > 1 && data.totalInvested > 0
      ? vh.map((p) => ({
          date: p.date,
          value:
            Math.round(
              ((p.value - data.totalInvested) / data.totalInvested) * 10000,
            ) / 100,
        }))
      : [];
  const dayChangeHistory =
    vh.length > 1
      ? vh
          .map((p, i) => ({
            date: p.date,
            value:
              i === 0 ? 0 : Math.round((p.value - vh[i - 1].value) * 100) / 100,
          }))
          .slice(1)
      : [];

  const plColor =
    combinedPnL == null || combinedPnL >= 0 ? "#34d399" : "#f87171";
  const roiChartColor =
    data.totalROI == null || data.totalROI >= 0 ? "#34d399" : "#f87171";

  const fmtChange = (v) =>
    v != null ? (v >= 0 ? "+" : "−") + format(Math.abs(v)) : "—";
  const fmtPct = (v) =>
  ...
  return (
    <>
      <div
        className="grid grid-cols-3 gap-3 px-6 pb-2"
      ...
      >
        ...
        <Widget
          ...
          onExpand={() =>
            expand({
              ...
              color: "#f59e0b",
              gradId: "pw-val",
              currentValue: fmt(data.totalValue),
              subtitle: `${data.cardsWithPrice} cards with price data`,
        ...
        >
          <div className="inline-flex flex-col items-center">
            ...
            <p className="text-slate-400 text-xs mt-0.5">
              {data.cardsWithPrice} card{data.cardsWithPrice !== 1 ? "s" : ""}{" "}
              with data
            </p>
          </div>
        </Widget>
        ...
        <Widget
          ...
          chart={dayChangeHistory}
          gradId="pw-day"
          color={dayPos ? "#34d399" : dayNeg ? "#f87171" : "#64748b"}
          formatter={fmtChange}
          zeroLine={true}
          onExpand={
            dayChangeHistory.length > 1
              ? () =>
                  expand({
                    label: "Today's Change",
                    chart: dayChangeHistory,
        ...
        <div className="relative bg-surface-800 border border-surface-600 rounded-xl p-2 pl-5 overflow-hidden flex flex-col justify-start">
          <p className="text-slate-500 text-xs mb-1 uppercase tracking-wider font-medium">
            Cards Tracked
          </p>
          <div className="flex-1 flex flex-col justify-center">
            <div className="flex items-center gap-0">
              ...
              >
                <p
                  className={`text-2xl font-bold leading-tight ${data.upAlertCount > 0 ? "text-emerald-400" : "text-slate-500"}`}
                >
                  {data.upAlertCount ?? 0}
                </p>
              ...
              >
                <p
                  className={`text-2xl font-bold leading-tight ${data.downAlertCount > 0 ? "text-red-400" : "text-slate-500"}`}
                >
        ...
        <Widget
          ...
          chart={data.investedHistory || []}
          gradId="pw-inv"
          color="#64748b"
          formatter={(v) => format(v)}
          addedDates={investedAddedDates}
          onExpand={
            data.investedHistory?.length > 1
              ? () =>
                  expand({
                    label: "Total Invested",
                    chart: data.investedHistory,
        ...
        >
          <div className="inline-flex flex-col items-center">
            ...
            <p className="text-slate-400 text-xs mt-0.5">
              {data.cardsWithCost} of {data.portfolioCount} card
        ...
        >
          <div className="inline-flex flex-col items-center">
            <p className={`text-2xl font-bold ${profitColor}`}>
              {combinedPnL != null ? (combinedPnL >= 0 ? "+" : "−") : ""}
              {fmt(combinedPnL != null ? Math.abs(combinedPnL) : null)}
            </p>
            <p className="text-slate-400 text-xs mt-0.5">
              {hasRealized ? "unrealized + realized" : "vs cost basis"}
            </p>
          </div>
        </Widget>
