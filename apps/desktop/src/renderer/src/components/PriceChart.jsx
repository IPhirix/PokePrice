...
import { fillDailyGaps } from "../utils/priceHistory";
...
export default function PriceChart({
  ...
  const gapFilledHistory = useMemo(() => fillDailyGaps(history), [history]);

  // Prepend 90 days of synthetic data before the first real data point
  const combined = useMemo(() => {
    if (!gapFilledHistory || gapFilledHistory.length === 0) return [];
    const synthetic = generateSynthetic(
      gapFilledHistory[0].price,
      gapFilledHistory[0].date,
      90,
    );
    return [...synthetic, ...gapFilledHistory];
  }, [gapFilledHistory]);
        ...
        {showRangeButtons && (
          <div className="flex items-center gap-1 mb-4 flex-wrap flex-shrink-0">
            ...
            {conditionSlot ? (
              ...
              <span className="ml-auto text-slate-700 text-xs self-center">
                No data yet
              </span>
            )}
          </div>
  ...
  // Explicit, evenly-spaced ticks by index (data is one entry per calendar day
  // after gap-filling, so evenly-spaced indices are also evenly-spaced dates).
  // Always includes the first and last point.
  const desiredTickCount = Math.min(8, data.length);
  const tickValues = (() => {
    if (data.length <= desiredTickCount) return data.map((d) => d.date);
    const step = (data.length - 1) / (desiredTickCount - 1);
