...
import { buildPokemonCardQuery } from '../utils/pokemonSearchQuery'
...
// rawPrice and parseCardNum imported from pokemonDetailUtils
...
function PokemonDetail({ pokemon, ownedCards, onBack, onRefreshOwned, favorites, onToggleFavorite }) {
  ...
  useEffect(() => {
    ...
    setCards([])
    const query = buildPokemonCardQuery(pokemon.displayName)
  ...
  useEffect(() => {
    ...
    async function worker() {
      while (queue.length && !cancelled) {
        ...
        try {
          const detail = await window.api.getCardById(card.id)
          if (!cancelled && detail) {
            const { rarity, artist, types, variants, set } = detail
  ...
  const uniqueSets = useMemo(() => buildSetDropdownOptions(cards), [cards])
  ...
  const visibleCards = useMemo(() => {
    ...
    return sortCards(base, sortBy, sortDir)
