...
const VARIANT_BADGE_LIST = [
  { key: "normal", label: "Normal", cls: "bg-slate-600/80 text-slate-100" },
  {
    key: "firstEdition",
    label: "1st Ed",
    cls: "bg-yellow-700/80 text-yellow-100",
  },
  { key: "holo", label: "Holo", cls: "bg-blue-700/80 text-blue-100" },
  {
    key: "reverse",
    label: "Rev. Holo",
    cls: "bg-purple-700/80 text-purple-100",
  },
  {
    key: "wPromo",
    label: "W Promo",
    cls: "bg-emerald-700/80 text-emerald-100",
  },
];

function VariantBadges({ variants, className = "" }) {
  const active = variants
    ? VARIANT_BADGE_LIST.filter((v) => variants[v.key])
    : [];
  ...
  return (
    <div className={`flex flex-wrap gap-0.5 items-center ${className}`}>
      {active.map((v) => (
        <span
          key={v.key}
          className={`text-[10px] font-semibold px-1 py-0.5 rounded leading-none ${v.cls}`}
        >
          {v.label}
        </span>
      ))}
...
export function CardDetailModal({
  ...
  function toggleFav() {
    const f = getFavs();
    if (f[favKey]) delete f[favKey];
    else f[favKey] = card.name;
    saveFavs(f);
    window.api
      .appendActivity({
  ...
  const inPortfolio = ownedCards.some(
    (c) => c.tcgId === card.id && c.section === "collection",
  );
  const inWatchlist = ownedCards.some(
    (c) => c.tcgId === card.id && (!c.section || c.section === "watchlist"),
  );
  ...
  const dayPct =
    dayDollar != null && prevPrice > 0 ? (dayDollar / prevPrice) * 100 : null;
  ...
  useEffect(() => {
    ...
    if (ownedEntry?.id) {
      ...
      if (ownedEntry.recentHistory?.length)
        setPriceHistory(ownedEntry.recentHistory);
      else setPriceHistory([]);
      window.api
        .getPriceHistory(ownedEntry.id)
        ...
        <div className="bg-surface-800 border border-surface-600 rounded-2xl w-full max-w-4xl overflow-hidden flex max-h-[96vh]">
          ...
          <div className="flex-shrink-0 w-[320px] bg-surface-900 flex flex-col relative overflow-hidden">
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(to top, rgba(245,158,11,0.10) 0%, transparent 100%)",
              }}
            />
            <div className="flex-1 min-h-0 p-6 flex items-center justify-center">
              <img
                ...
                alt={card.name}
                className="max-h-[420px] w-full object-contain rounded-xl select-none cursor-pointer relative z-10"
                style={{
            ...
            <div className="flex items-center justify-between px-6 pt-5 pb-[14px] flex-shrink-0 border-b border-surface-700/60">
              <div className="flex items-center gap-2 min-w-0 pr-2">
                <button
                  onClick={toggleFav}
                  title={
                    isFav ? `Unfavorite ${card.name}` : `Favorite ${card.name}`
                  }
                ...
                <h2 className="text-[18px] font-bold text-white leading-tight tracking-[-0.02em]">
                  {card.name}
                  {card.number ? (
                    <span className="text-[14px] font-normal text-slate-400 ml-1.5">
                      #{card.number}
                    </span>
                  ) : null}
                </h2>
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
              ...
              <div className="px-6 py-4 border-b border-surface-700/60 flex items-end gap-4 flex-shrink-0">
                <div>
                  ...
                  {price != null ? (
                    ...
                    <p className="text-[36px] font-extrabold text-slate-600 leading-none">
                      —
                    </p>
                  )}
                </div>
                {dayPct != null && (
                  <span
                    className={`text-[13px] font-bold tabular-nums pb-1 ${dayPct >= 0 ? "text-emerald-400" : "text-red-400"}`}
                  >
                    {dayPct >= 0 ? "+" : ""}
                    {dayPct.toFixed(1)}%
                  </span>
                )}
                <span className="ml-auto text-[11px] text-slate-600 pb-1 self-end">
                  Updated today
                </span>
              </div>
              ...
              <div className="px-6 flex-shrink-0">
                <div className="flex items-baseline justify-between py-[9px] border-b border-surface-700/40">
                  <span className="text-[11px] font-medium text-slate-500 uppercase tracking-[0.05em] flex-shrink-0 mr-4">
                    Set
                  </span>
        ...
        >
          <img
            ...
            style={{
              ...
              transition:
                inspectTilt.x === 0 && inspectTilt.y === 0
                  ? "transform 0.4s ease"
                  : "transform 0.05s linear",
            }}
      ...
      if (newCard?.id && !targets.alertPrice) {
        ...
        if (latestPrice != null) {
          if (settings.defaultAlertUpPct != null) {
            targets.alertPrice =
              Math.round(
                latestPrice * (1 + settings.defaultAlertUpPct / 100) * 100,
              ) / 100;
            targets.alertPct = settings.defaultAlertUpPct;
          } else if (settings.defaultAlertDownPct != null) {
            targets.alertPrice =
              Math.round(
                latestPrice * (1 - settings.defaultAlertDownPct / 100) * 100,
              ) / 100;
...
export default function SearchPage({
  ...
  async function loadPopular() {
    ...
    setHasSearched(true);
    try {
      const cards = await window.api.searchCardsAdvanced(
  ...
  function handleCardAdded() {
    ...
    loadOwnedCards();
    window.api
      .listBinders("collection")
      .then(setPortfolioBinders)
      .catch(() => {});
    window.api
      .listBinders("watchlist")
      .then(setWatchlistBinders)
      .catch(() => {});
  }
  ...
  useEffect(() => {
    ...
    setResults([]);
    window.api
      .searchCardsAdvanced(`set.id:"${browsedSet.id}"`)
      .then((cards) => {
        ...
        const withSeries = series
          ? cards.map((c) =>
              c._divider
                ? c
                : { ...c, set: { ...c.set, series: c.set?.series || series } },
            )
          : cards;
  ...
  useEffect(() => {
    ...
    async function worker() {
      while (queue.length && !cancelled) {
        ...
        try {
          const fetched = await window.api.searchCardsAdvanced(
            `id:"${card.id}"`,
          );
  ...
  async function runSearch(
    ...
    if (cType === "Pokemon") extraParts.push(`supertype:"Pokémon"`);
      ...
      extraParts.push(`supertype:"Trainer" subtypes:"Item"`);
    else if (cType === "Supporter")
      extraParts.push(`supertype:"Trainer" subtypes:"Supporter"`);
    else if (cType === "Stadium")
      extraParts.push(`supertype:"Trainer" subtypes:"Stadium"`);
    else if (cType === "Tool")
      extraParts.push(`supertype:"Trainer" subtypes:"Tool"`);
    ...
    const searchWith = (queryStr) =>
      window.api
        .searchCardsAdvanced([queryStr, ...extraParts].join(" "))
        .catch(() => []);
    ...
    try {
      ...
      if (!q) {
        ...
        merged = await window.api
          .searchCardsAdvanced(extraParts.join(" "))
          .catch(() => []);
      } else {
        ...
        if (hashMatch) {
          ...
          if (namePart) {
            ...
            for (const c of allCards) {
              ...
              const hits = isNumeric
                ? parseInt(cn, 10) === targetNum
                : cn.toUpperCase() === rawNum.toUpperCase();
            ...
            const numVariants = isNumeric
              ? [
                  ...new Set([
                    rawNum,
                    String(targetNum),
                    String(targetNum).padStart(3, "0"),
                  ]),
                ]
              : [rawNum];
            const seen = new Set();
            const batches = await Promise.all(
              numVariants.map((v) => searchWith(`number:"${v}"`)),
            );
          ...
          if (trailingMatch) {
            ...
            const [allCards, setCards] = await Promise.all([
              ...
              hasSetFilter
                ? Promise.resolve([])
                : searchWith(`name:"${namePart}*" set.name:"${rawNum}"`),
            ]);
            const seen = new Set();
            const matched = [];
            const rest = [];
            for (const c of allCards)
              if (
                parseInt(String(c.number || ""), 10) === targetNum &&
                !seen.has(c.id)
              ) {
                seen.add(c.id);
                matched.push(c);
            ...
            if (!hasSetFilter) {
              ...
              for (let i = 1; i < words.length; i++) {
                ...
                allSearches.push(
                  searchWith(`name:"${namePart}*" set.name:"${setPart}"`),
                );
              }
            }
            const seen = new Set();
  ...
  async function openCardModal(card) {
    ...
    if (card.id && !card.rarity && !card.artist && !card.types?.length) {
      const results = await window.api
        .searchCardsAdvanced(`id:"${card.id}"`)
        .catch(() => []);
  ...
  async function runSealedSearch(queryOverride, isSilent = false) {
    ...
    try {
      ...
      if (err === "no_token") {
        if (!isSilent)
          setError("PriceCharting API token required — add it in Settings.");
      } else {
  ...
  const isSealedQuery = SEALED_KEYWORDS.some((k) =>
    nameQuery.trim().toLowerCase().includes(k.toLowerCase()),
  );
  const hideCardsForSealed =
    isSealedQuery && (sealedLoading || sealedResults.length > 0);

  const binderResults = binderFilter
    ? ownedCards
        .filter((c) => {
          if (binderFilter === "collection") return c.section === "collection";
          if (binderFilter === "watchlist")
  ...
  const baseResults =
    mode === "sets" && browsedSet ? results : (binderResults ?? results);
  ...
  const allYears = useMemo(() => {
    const y = new Set(
      allSets
        .map((s) => (s.releaseDate || "").split("/")[0].split("-")[0])
        .filter(Boolean),
    );
    return Array.from(y).sort().reverse();
  }, [allSets]);

  const setDateMap = useMemo(
    () => new Map(allSets.map((s) => [s.id, s.releaseDate || ""])),
    [allSets],
  );
  ...
  const uniqueArtists = useMemo(() => {
    const vals = results
      .filter((c) => !c._divider)
      .map((c) => cardDetails[c.id]?.artist || c.artist)
      .filter(Boolean);
  ...
  const setsFiltered = useMemo(() => {
    ...
    return [...allSets]
      .filter((s) => {
        ...
        if (nameLower.includes("mcdonald") || seriesLower.includes("mcdonald"))
          return false;
  ...
  const allBinders = useMemo(() => {
    ...
    portfolioBinders.forEach((name) => {
      const cards = ownedCards.filter(
        (c) => (c.binder || c.folder) === name && c.section === "collection",
      );
      const coverImages = cards
        .map((c) => c.imageUrl)
        .filter(Boolean)
        .slice(0, 4);
      entries.push({
        ...
        section: "collection",
        count: cards.length,
        value: cards.reduce((s, c) => s + (c.currentPrice ?? 0), 0),
        coverImages,
      });
    });
  ...
  const browsedBinderCards = useMemo(() => {
    ...
    return ownedCards
      .filter((c) => {
        ...
        if (browsedBinder.section === "collection")
          return c.section === "collection";
        return !c.section || c.section === "watchlist";
      })
      .map((c) => ({
        id: c.tcgId,
  ...
  const bindersFiltered = useMemo(() => {
    ...
    return allBinders
      .filter((f) => {
        ...
        if (binderSectionFilter && f.section !== binderSectionFilter)
          return false;
        return true;
      })
      .sort((a, b) => {
  ...
  const sortCompareFn = (a, b) => {
    ...
    if (sortOrder === "price")
      return (cardPrice(b) ?? -Infinity) - (cardPrice(a) ?? -Infinity);
    if (sortOrder === "price_low")
      return (cardPrice(a) ?? Infinity) - (cardPrice(b) ?? Infinity);
  ...
  const displayResults = useMemo(() => {
    ...
    if (variationFilter) {
      base = base.filter(
        (card) => card._divider || card.variants?.[variationFilter] === true,
      );
    }
    if (illustratorFilter) {
      base = base.filter(
        (card) => card._divider || card.artist === illustratorFilter,
      );
    }
    ...
    return base.filter((card) => {
      ...
      if (setCollectionFilter === "owned")
        return ownedCards.some((c) => c.tcgId === card.id);
      if (setCollectionFilter === "collection")
        return ownedCards.some(
          (c) => c.tcgId === card.id && c.section === "collection",
        );
      if (setCollectionFilter === "watchlist")
        return ownedCards.some(
          (c) =>
            c.tcgId === card.id && (!c.section || c.section === "watchlist"),
        );
      if (setCollectionFilter === "not_owned")
        return !ownedCards.some((c) => c.tcgId === card.id);
      return true;
    });
  }, [
  ...
  return (
    <div>
            ...
            {mode === "cards" && (
              <>
                {artistFilter && (
                  <div className="w-full flex items-center gap-2 bg-amber-900/20 border border-accent/40 rounded-lg px-3 py-2">
                    ...
                    <span className="text-accent text-sm">
                      Illustrator:{" "}
                      <strong className="text-white">{artistFilter}</strong>
                    </span>
                    <button
                      onClick={() => {
                        setArtistFilter("");
                ...
                <div className="flex-1 min-w-[160px]">
                  <label className="text-slate-400 text-xs mb-1.5 block uppercase tracking-wider font-medium">
                    Pokémon Name
                  </label>
                  <input
                    ref={nameRef}
                ...
                >
                  ...
                  >
                    <span
                      className={setQuery ? "text-white" : "text-slate-500"}
                    >
                      {setQuery || "Any set…"}
                    </span>
                    <svg
                      className={`w-4 h-4 text-slate-400 flex-shrink-0 transition-transform ${setDropdownOpen ? "rotate-180" : ""}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
          ...
          {mode === "cards" && (
            <div className="flex gap-3 items-end flex-wrap">
              ...
              <div className="w-48 flex-shrink-0">
                ...
                <select
                  value={illustratorFilter}
                  onChange={(e) => setIllustratorFilter(e.target.value)}
                  className="w-full bg-surface-700 border border-surface-500 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-accent"
                >
                    ...
                    <div className="relative bg-surface-900 h-32 flex items-center justify-center p-4">
                      {releaseDate && (
                        <div className="absolute top-2 right-2 text-right">
                          <p className="text-slate-500 text-[10px] uppercase tracking-wider leading-none mb-1">
                            Released
                          </p>
                        ...
                        <div className="mb-1.5">
                          <div className="flex items-center justify-between mb-0.5">
                            ...
                            <span className="text-white text-xs font-medium">
                              {stats.count}/{totalCards}{" "}
                              <span className="text-slate-500">
                                (
                                {totalCards > 0
                                  ? Math.round((stats.count / totalCards) * 100)
                                  : 0}
                                %)
                              </span>
                            </span>
                          ...
                          <div className="h-1.5 bg-surface-700 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(100, totalCards > 0 ? (stats.count / totalCards) * 100 : 0)}%`,
                              }}
                            />
                          </div>
                        </div>
        ...
        <div>
          {allBinders.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-slate-600">
              ...
              <p className="text-sm">
                Add cards to binders from your Collection or Watchlist
              </p>
            </div>
                ...
                >
                  ...
                  >
                    {binder.coverImages.length === 0 ? (
                      ...
                      <div
                        className={`grid h-full ${binder.coverImages.length === 2 ? "grid-cols-2" : "grid-cols-2 grid-rows-2"}`}
                      >
                    ...
                    <div className="absolute top-3 right-3">
                      {binder.section === "collection" ? (
                        <span className="bg-accent text-black text-[11px] font-bold px-2.5 py-1 rounded-full shadow">
                          Collection
                        </span>
                      ) : (
                        <span className="bg-sky-500 text-white text-[11px] font-bold px-2.5 py-1 rounded-full shadow">
                          Watchlist
                        </span>
                      )}
                    </div>
                    ...
                    <div className="absolute bottom-3 left-3 right-3">
                      <p className="text-white font-bold text-base leading-tight truncate drop-shadow-lg">
                        {binder.name}
                      </p>
                  ...
                  <div className="px-4 py-3 flex items-center justify-between gap-2">
                    ...
                    <p className="text-accent font-bold text-sm flex-shrink-0">
                      {format(binder.value)}
                    </p>
                  </div>
                </button>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-slate-500 text-xs">
              {browsedBinder.section === "collection"
                ? "Collection"
                : "Watchlist"}{" "}
              · {browsedBinderCards.length}{" "}
              {browsedBinderCards.length === 1 ? "card" : "cards"}
            </p>
          </div>
          {browsedBinder.section === "collection" ? (
            <span className="bg-accent/20 text-accent text-xs font-bold px-2 py-1 rounded-full flex-shrink-0">
              Collection
            </span>
          ) : (
            <span className="bg-sky-500/20 text-sky-400 text-xs font-bold px-2 py-1 rounded-full flex-shrink-0">
              Watchlist
            </span>
          )}
          {confirmDeleteBinder ? (
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button
                onClick={async () => {
                  await window.api.deleteBinder(
                    browsedBinder.section,
                    browsedBinder.name,
                  );
          ...
          {!sealedLoading &&
            ...
            !error && (
              <div className="flex flex-col items-center justify-center py-24 text-slate-600">
                ...
                <p className="text-sm">
                  Elite Trainer Boxes, Booster Boxes, and more
                </p>
                <p className="text-xs mt-2 text-slate-700">
                  Requires PriceCharting API token (set in Settings)
                </p>
              </div>
            )}
            ...
            <div className="flex flex-col gap-2">
              {sealedResults.map((product) => {
                ...
                const ownedCount = ownedCards.filter(
                  (c) => c.name === (product.name || product["product-name"]),
                ).length;
                ...
                return (
                  ...
                  >
                    <div className="w-14 h-14 flex-shrink-0 bg-surface-700 rounded-lg overflow-hidden relative flex items-center justify-center">
                      <svg
                        className="w-7 h-7 text-slate-500 absolute"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button
                        onClick={() =>
                          setSealedAddModal({ product, section: "collection" })
                        }
                      ...
                      <button
                        onClick={() =>
                          setSealedAddModal({ product, section: "watchlist" })
                        }
      ...
      {mode === "cards" &&
        binderFilter &&
        !loading &&
        binderResults.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-slate-600">
            <p className="text-lg">
              No cards in{" "}
              {binderFilter.startsWith("binder:")
                ? binderFilter.slice(7)
                : binderFilter === "collection"
                  ? "Collection"
                  : "Watchlist"}
            </p>
            <p className="text-sm mt-1">
              Add cards from the search results above
            </p>
          </div>
        )}
      ...
      {mode === "folders" &&
        browsedBinder &&
        orderedBinderCards.length > 0 &&
        (() => {
          const SLOTS = 9;
          const pageCount = Math.ceil(orderedBinderCards.length / SLOTS);
          const pageSlice = orderedBinderCards.slice(
            browsedBinderPage * SLOTS,
            (browsedBinderPage + 1) * SLOTS,
          );
          ...
          function handleDrop(e, absoluteIdx) {
            ...
            if (srcIdx == null || srcIdx === absoluteIdx) {
              setDragOverIdx(null);
              return;
            }
          ...
          return (
            <div className="flex flex-col items-center gap-5">
              <p className="text-slate-600 text-xs self-end">
                  ...
                  {Array.from({ length: SLOTS }).map((_, slotIdx) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          opacity: dragSrcRef.current === absoluteIdx ? 0.4 : 1,
                        }}
                      >
                        {card.images?.small ? (
                          <img
                          ...
                          <div className="w-full h-full bg-surface-700 flex items-center justify-center text-slate-600 text-[10px]">
                            No img
                          </div>
                        )}
                        {(inPortfolio || inWatchlist) && (
                          <div className="absolute top-1 right-1 flex flex-col gap-0.5 pointer-events-none">
                            {inPortfolio && (
                              <span className="text-[7px] font-bold px-1 py-0.5 rounded bg-accent text-black leading-none">
                                Coll.
                              </span>
                            )}
                            {inWatchlist && (
                              <span className="text-[7px] font-bold px-1 py-0.5 rounded bg-sky-500 text-white leading-none">
                                Watch.
                              </span>
                            )}
                          </div>
              ...
              {pageCount > 1 && (
                <div className="flex items-center gap-3">
                  <button
                    onClick={() =>
                      setBrowsedBinderPage((p) => Math.max(0, p - 1))
                    }
                  ...
                  <span className="text-slate-500 text-sm">
                    Page {browsedBinderPage + 1} of {pageCount}
                  </span>
                  <button
                    onClick={() =>
                      setBrowsedBinderPage((p) =>
                ...
                {displayResults.map((card) => {
                  if (card._divider) {
                    return (
                      <div
                        key="__divider__"
                        className="col-span-full flex items-center gap-3 py-2 select-none"
                      >
                  ...
                  const portfolioCount = ownedCards.filter(
                    (c) => c.tcgId === card.id && c.section === "collection",
                  ).length;
                  const watchlistCount = ownedCards.filter(
                    (c) =>
                      c.tcgId === card.id &&
                      (!c.section || c.section === "watchlist"),
                  ).length;
                  ...
                  const isFav = favNames.some((n) => {
                    ...
                    return fn && (cn === fn || cn.includes(fn));
                  });
                  return (
                    <div
                      key={card.id}
                      onClick={() => openCardModal(card)}
                      className="relative border border-surface-600 hover:border-surface-400 rounded-xl p-2 bg-surface-800 transition-colors cursor-pointer"
                    >
                      {isFav && (
                        <span className="absolute top-2 left-2 text-yellow-400 text-xl leading-none pointer-events-none z-10">
                          ★
                        </span>
                      )}
                      {(inPortfolio || inWatchlist) && (
                        <div className="absolute top-1.5 right-1.5 flex flex-col gap-0.5 z-10 pointer-events-none">
                          {inPortfolio && (
                            <span className="text-[11px] font-bold px-2 py-1 rounded bg-accent text-black leading-none">
                              Collection
                              {portfolioCount > 1 ? ` ×${portfolioCount}` : ""}
                            </span>
                          )}
                          {inWatchlist && (
                            <span className="text-[11px] font-bold px-2 py-1 rounded bg-sky-500 text-white leading-none">
                              Watchlist
                      ...
                      <div className="aspect-[5/7] w-full">
                        ...
                        <div
                          style={{
                            display: card.images?.small ? "none" : "flex",
                          }}
                          className="w-full h-full bg-surface-700 rounded-lg flex-col items-center justify-center text-slate-600 gap-1"
                        >
                          <svg
                            width="28"
                            height="36"
                      ...
                      <div className="mt-2 px-0.5">
                        ...
                        />
                        {price != null && (
                          <p className="text-slate-300 text-xs font-semibold text-center mt-1">
                            {format(price)}
                          </p>
                        )}
                      </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-surface-700">
                      <th className="text-left text-slate-500 text-xs uppercase tracking-wider px-4 py-3 font-medium">
                        Card
                      </th>
                      <th className="text-left text-slate-500 text-xs uppercase tracking-wider px-4 py-3 font-medium">
                        Set
                      ...
                      <th className="text-right text-slate-500 text-xs uppercase tracking-wider px-4 py-3 font-medium">
                        Price
                      </th>
                      <th className="text-right text-slate-500 text-xs uppercase tracking-wider px-4 py-3 font-medium">
                        Actions
                  ...
                  <tbody>
                    {displayResults.map((card) => {
                      ...
                      const portfolioCount = ownedCards.filter(
                        (c) =>
                          c.tcgId === card.id && c.section === "collection",
                      ).length;
                      const watchlistCount = ownedCards.filter(
                        (c) =>
                          c.tcgId === card.id &&
                      ...
                      return (
                        <tr
                          key={card.id}
                          onClick={() => openCardModal(card)}
                          className="border-b border-surface-700 last:border-0 hover:bg-surface-700/50 cursor-pointer transition-colors"
                        >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="flex gap-1 mt-1">
                                  {inPortfolio && (
                                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-accent text-black leading-none">
                                      Collection
                                      {portfolioCount > 1
                                        ? ` ×${portfolioCount}`
                                        : ""}
                                  ...
                                  {inWatchlist && (
                                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-sky-500 text-white leading-none">
                                      Watchlist
                                      {watchlistCount > 1
                                        ? ` ×${watchlistCount}`
                                        : ""}
                          ...
                          <td className="px-4 py-2.5">
                            <VariantBadges variants={card.variants} />
                          </td>
                          ...
                          >
                            <div className="flex gap-1.5 justify-end">
                              {inPortfolio ? (
                                <span className="text-xs text-accent font-semibold px-2 py-1 rounded bg-accent/10 border border-accent/30">
                                  ✓ Collection
                                </span>
                              ) : (
                                <button
                                  onClick={() =>
                                    setAddModal({ card, section: "collection" })
                                  }
                                  className="text-xs font-bold px-2 py-1 rounded bg-accent hover:bg-accent-hover text-black transition-colors whitespace-nowrap"
                                >
                                  + Collection
                                </button>
                              )}
                              {inWatchlist ? (
                                <span className="text-xs text-sky-400 font-semibold px-2 py-1 rounded bg-sky-500/10 border border-sky-500/30">
                                  ✓ Watchlist
                                </span>
                              ) : (
                                <button
                                  onClick={() =>
                                    setAddModal({ card, section: "watchlist" })
                                  }
                                  className="text-xs font-bold px-2 py-1 rounded bg-sky-600 hover:bg-sky-500 text-white transition-colors whitespace-nowrap"
                                >
                                  + Watchlist
                                </button>
                              )}
      ...
      {/* ── Sealed results — rendered inline with card results, matching the active view mode ── */}
      {mode === "cards" &&
        (sealedLoading || sealedResults.length > 0) &&
        nameQuery.trim() && (
          <div className={baseResults.length > 0 ? "mt-4" : "mt-0"}>
            {sealedLoading && (
            ...
            {!sealedLoading && baseResults.length > 0 && (
              <div className="flex items-center gap-3 py-2 mb-2 select-none">
                <div className="flex-1 h-px bg-surface-600" />
                <span className="text-xs text-slate-500 font-medium">
                  Sealed Products
                </span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                {sealedResults.map((product) => {
                  ...
                  return (
                    <div
                      key={product.tcgPlayerId || product.id}
                      className="relative border border-surface-600 hover:border-surface-400 rounded-xl p-2 bg-surface-800 transition-colors"
                    >
                      {alreadyOwned && (
                        <div className="absolute top-1.5 right-1.5 z-10 pointer-events-none">
                          <span className="text-[11px] font-bold px-2 py-1 rounded bg-emerald-500/20 text-emerald-400 leading-none">
                            Owned{ownedCount > 1 ? ` ×${ownedCount}` : ""}
                          </span>
                        </div>
                      )}
                      <div className="aspect-[5/7] w-full bg-surface-700 rounded-lg overflow-hidden relative flex items-center justify-center">
                        <svg
                          className="w-7 h-7 text-slate-500 absolute"
                          fill="none"
                          viewBox="0 0 24 24"
                      ...
                      <div className="flex gap-1.5 mt-2">
                        <button
                          ...
                          className="flex-1 px-2 py-1 bg-accent hover:bg-accent-hover text-black text-[11px] font-bold rounded transition-colors whitespace-nowrap"
                        >
                          + Collection
                        </button>
                        <button
                          ...
                          className="flex-1 px-2 py-1 bg-sky-600 hover:bg-sky-500 text-white text-[11px] font-bold rounded transition-colors whitespace-nowrap"
                        >
                          + Watchlist
                        </button>
                      </div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {sealedResults.map((product) => {
                      ...
                      const ownedCount = ownedCards.filter(
                        (c) =>
                          c.name === (product.name || product["product-name"]),
                      ).length;
                      const alreadyOwned = ownedCount > 0;
                      const imgSrc =
                        product.imageUrl ||
                      ...
                      return (
                        ...
                        >
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-14 flex-shrink-0 bg-surface-700 rounded overflow-hidden relative flex items-center justify-center">
                                {imgSrc && (
                                  <img
                                    src={imgSrc}
                                    alt={productName}
                              ...
                              <div>
                                ...
                                {alreadyOwned && (
                                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 leading-none">
                                    Owned
