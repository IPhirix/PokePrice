...
const TABS = [
  ...
  { id: 'trade',     label: 'Trade Analyzer',    color: 'text-rose-400',     activeBg: 'bg-rose-900/20 border-rose-500' },
...
const COND_LABEL_EXPORT = CONDITION_LABEL
...
const SOLD_COND_LABEL = CONDITION_LABEL
const SOLD_COND_COLOR = CONDITION_COLOR
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh prices on startup if stale (no update today)
    window.api.getSettings().then((s) => {
      const lastRefreshed = s?.lastRefreshed
      const stale = !lastRefreshed || (Date.now() - new Date(lastRefreshed).getTime()) > 23 * 60 * 60 * 1000
      if (stale) handleRefreshAllPrices()
    }).catch(() => {})
  ...
  async function handleRefreshAllPrices() {
    ...
    try {
      ...
      loadCards()
      setRefreshMsg({ ok: true, text: `Updated ${result?.refreshed ?? 0} card${result?.refreshed !== 1 ? 's' : ''}` })
    } catch (e) {
